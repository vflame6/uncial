...
public enum CodeHighlighter {
    ...
    /// One highlight.js instance per process, created on the first fence. JavaScriptCore serializes
    /// calls on a context; the lock keeps whole highlight calls from interleaving on top of that.
    private final class Engine: @unchecked Sendable {
        ...
        func supports(_ language: String) -> Bool {
            lock.lock()
            defer { lock.unlock() }
            if let known = known[language] { return known }
    ...
    public static func supports(_ language: String) -> Bool {
        ...
        return engine?.supports(name) ?? false
    }

    /// Replaces the content of every `<pre><code class="language-…">` whose language the bundle knows
    /// with highlight.js's spans, balanced per line; other blocks stay as cmark wrote them.
    static func render(_ html: String) -> String {
        ...
        for match in fence.matches(in: html, range: NSRange(location: 0, length: text.length)) {
            ...
            if let highlighted = self.html(for: code, language: language) {
    ...
    /// line break), or nil for a language the bundle does not know. Cached per language and source.
    public static func html(for code: String, language: String) -> String? {
        let name = normalized(language)
        guard supports(name) else { return nil }
    ...
    public static func tokens(in code: String, language: String) -> [Token] {
        guard let html = html(for: code, language: language) else { return [] }
        var tokens: [Token] = []
        var stack: [Scope?] = []
        var offset = 0
