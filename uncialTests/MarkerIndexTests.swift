...
@Suite struct MarkerIndexTests {
    ...
    @Test func hidesResolvedDiagramBlocksButTheirLastNewline() {
        ...
        let plain = MarkerIndex(tokens: tokens)
        #expect(!plain.isHidden(17) && plain.isHidden(6))
        let resolved = MarkerIndex(tokens: tokens, resolvedDiagrams: [6])
        #expect(resolved.hidden == [block])
    ...
    @Test func hidesPicturedMathButItsAnchor() {
        ...
        let resolved = MarkerIndex(tokens: tokens, resolvedMath: [2: 2, 8: 13])
